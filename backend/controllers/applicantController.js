...
        // 1. Fetch KPI data based on the filter, using the CORRECT kebab-case status from Mongoose schema
        const totalApplicants = await Applicant.countDocuments(dateFilter);
        
        // ✅ FIXED: Using the Mongoose Schema enum values ('selected', 'not-selected', etc.)
...
            // ✅ FIXED: Ensure the action string is formatted with spaces and capitalization for display
            action: `updated status to ${a.status.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')}`, 
